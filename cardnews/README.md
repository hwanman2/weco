...
- `items: ['질문1', '질문2', ...]` 를 넣으면 인용문 목록 카드가 된다.
  `art: null` 로 두면 그림 없이 글만 채운다.
...
- 하단 워드마크는 기본이 `hwanman`. 세트에 `brand: '다른이름'` 또는
  `brand: ''`(끄기)를 넣어 바꾼다.
